    ...
    if not exportieren:
        ...
        if data:
            if suchparameter["ansicht_auswahl"] == 'SalesHeader':
                ...
                return frappe.render_template('templates/d365_export/d365_exp_salesline.html', {'invoices': data})
        else:
            return False
    else:
        if exist_d365export_folder():
            # SalesHeader
            ...
            for sinv in salesheader_raw_data:
                mark_as_exportet = frappe.db.sql("""UPDATE `tabSales Invoice` SET `rechnung_nach_d365_exportiert` = 1 WHERE `name` = '{sinv}'""".format(sinv=sinv["sinv"]), as_list=True)
            
            return True
    # fallback
    return False

def exist_d365export_folder():
    exist = frappe.db.sql("""SELECT COUNT(`name`) AS `qty` FROM `tabFile` WHERE `name` = 'Home/D365Export' AND `is_folder` = 1""", as_dict=True)
    if exist[0].qty != 1:
        new_folder = frappe.get_doc({
            "doctype": "File",
            "file_name": "d365Export",
            "folder": "Home",
            "is_folder": 1
        })
        new_folder.insert()
...
def make_d365_xlsx(salesheader_data, salesline_data):
    for sinv in salesheader_data:
        if sinv[9] != 'Externe Belegnummer':
            update_export_cb = frappe.db.sql("""UPDATE `tabSales Invoice` SET `rechnung_nach_d365_exportiert` = 1 WHERE `name` = '{sinv}'""".format(sinv=sinv[9]), as_list=True)
