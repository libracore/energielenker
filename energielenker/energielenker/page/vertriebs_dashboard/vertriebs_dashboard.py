...
def get_leads(gl=False):
    ...
    if not gl:
        ...
        for emp in emp_sets:
            data_subset = {
                "name": frappe.db.get_value("User", emp.lead_owner, "full_name"),
                "values": []
            }
            for month in range(1, 7):
                m_delta = month - 6
...
def get_quotations(quotation_status, qty=1):
    ...
    if not gl:
        ...
        for emp in emp_sets:
            data_subset = {
                "name": frappe.db.get_value("User", emp.quotation_owner, "full_name") or frappe.db.get_value("User", emp.fallback_owner, "full_name") or 'N/A',
                "values": []
            }
            for month in range(1, 7):
                m_delta = month - 6
